...
export function DistributeInsumoDialog({
  ...
  const availableParents = (consolidatedItems || []).filter(
    (parent) => parent.id && parent.id !== sourceItem?.id
  );

  return (
    <Dialog open={open && !!sourceItem} onOpenChange={onOpenChange}>
      {sourceItem && (
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
          <DialogHeader>
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60 shadow-xs">
            ...
            <div className="rounded-xl border border-border/80 bg-muted/25 p-3.5 space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-bold text-foreground">
                  Costo Unitario: <span className="text-red-600 dark:text-red-400">{formatCLP(unitCost)}</span>
                </span>
              </div>
              <div className="flex items-center justify-between pt-1 border-t border-border/40">
                <span className="text-sm font-bold text-foreground">{sourceItem.servicio}</span>
                <span className="text-xs font-semibold px-2 py-0.5 rounded-md bg-blue-100 text-blue-800 dark:bg-blue-950/50 dark:text-blue-300 border border-blue-200 dark:border-blue-800/60">
                  ...
                  {availableParents.map((parent) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] px-1.5 py-0.5 rounded bg-muted font-medium text-muted-foreground whitespace-nowrap">
                                Rendimiento: {parent.cantidad || 1} un
                              </span>
                            </div>
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            <div className="text-right">
                              <span className="text-[10px] text-muted-foreground block mb-0.5">Derivar:</span>
                              <div className="flex items-center gap-1">
                                <button
                                  type="button"
                                  onClick={() => {
                                    const curr = allocations[parent.id!] || 0;
                                    if (curr > 0) handleQtyChange(parent.id!, String(curr - 1));
                                  }}
                                  disabled={assigned <= 0}
                                  className="w-7 h-7 flex items-center justify-center rounded-lg border border-border bg-background hover:bg-muted text-foreground disabled:opacity-30 disabled:pointer-events-none font-bold text-xs"
                                >
                                  -
                                </button>
                                <Input
                                  ...
                                  className="h-7 w-14 text-center font-bold text-sm rounded-lg px-1 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                />
                                <button
                                  type="button"
                                  onClick={() => {
                                    const curr = allocations[parent.id!] || 0;
                                    if (remainingStock > 0) handleQtyChange(parent.id!, String(curr + 1));
                                  }}
                                ...
                                <span className="text-xs font-semibold text-muted-foreground ml-0.5">un</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        {assigned > 0 && (
                          <div className="mt-2 pt-2 border-t border-indigo-100 dark:border-indigo-900/30 flex items-center justify-between text-xs text-indigo-950 dark:text-indigo-200 font-medium">
                            <span>Costo que se sumará a {parent.servicio}:</span>
                            <span className="font-bold text-red-600 dark:text-red-400">+{formatCLP(itemCost)}</span>
                          </div>
                        )}
                      </div>
                    );
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <button
                        type="button"
                        onClick={() => {
                          if (standaloneSplit > 0) handleStandaloneChange(String(standaloneSplit - 1));
                        }}
                        disabled={standaloneSplit <= 0}
                        className="w-7 h-7 flex items-center justify-center rounded-lg border border-border bg-background hover:bg-muted text-foreground disabled:opacity-30 disabled:pointer-events-none font-bold text-xs"
                      >
                        -
                      </button>
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          if (remainingStock > 0) handleStandaloneChange(String(standaloneSplit + 1));
                        }}
                        disabled={remainingStock <= 0}
                        className="w-7 h-7 flex items-center justify-center rounded-lg border border-border bg-background hover:bg-muted text-foreground disabled:opacity-30 disabled:pointer-events-none font-bold text-xs"
                      >
